...
.. plot::
   ...
   dews = list(zip(dew_data.pressure, dew_data.dewpoint))
   temps = list(zip(temp_data.pressure, temp_data.temperature))
...
.. plot::
    ...
    dews = list(zip(barb_data.pressure, barb_data.dewpoint))
    barbs = list(zip(barb_data.wind_speed, barb_data.wind_direction, barb_data.pressure))
    tpg = tephi.TephiAxes()
    profile = tpg.plot(dews)
    profile.barbs(barbs)
    plt.show()
